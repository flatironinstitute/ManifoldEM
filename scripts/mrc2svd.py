...
def op(svd_dir, proj_name, user_dir):
    ...
    for bin in range(p.nClass):
        rec_file = os.path.join(outputsDir,
                                'post/1_vol/EulerAngles_{}_{}_of_{}.mrc'.format(p.trajName, bin + 1, p.nClass))
    ...
    for bin in range(p.nClass):
        rec1_file = os.path.join(outputsDir,
                                 'post/2_svd/SVDimgsRELION_{}_{}_of_{}.mrc'.format(p.trajName, bin + 1, p.nClass))
        mrc = mrcfile.new(rec1_file)
        mrc.set_data(ConImgT[bin, :, :, :])
        print('Output volume:', (bin + 1))

    for t in range(topoNum):
        toporec_file = os.path.join(outputsDir,
                                    'post/2_svd/SVDTOPOimgsRELION_{}_{}_of_{}.mrc'.format(p.trajName, t + 1, p.nClass))
