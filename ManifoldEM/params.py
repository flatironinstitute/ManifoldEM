...
class Params():
    ...
    project_name: Annotated[str, ParamInfo('Name of the project', False, [ProjectLevel.INIT])] = ''
    project_level: Annotated[ProjectLevel, ParamInfo('Current analysis level of project')] = ProjectLevel.INIT
    is_relion_data: Annotated[bool, ParamInfo('Is the code analyzing relion data?')] = True
    ...
    eps: Annotated[
        float,
        ParamInfo('Small fraction to be added if divide-by-zero errors occur', True, [ProjectLevel.BINNING])] = 1E-10

    # microscopy parameters:
    ms_spherical_aberration: Annotated[float, ParamInfo('Spherical Aberration [mm] (from alignment file)')] = 0.0
    ms_kilovolts: Annotated[float, ParamInfo('Voltage [kV] (from alignment file)')] = 0.0
    ms_amplitude_contrast_ratio: Annotated[float, ParamInfo('Amplitude Contrast [ratio](from alignment file)')] = 0.0
    ms_ctf_envelope: Annotated[float, ParamInfo('Envelope of CTF')] = float("inf")
    ms_num_pixels: Annotated[int, ParamInfo('Window size of image (e.g., for 100x100 image, nPix=100)')] = 0
    ms_pixel_size: Annotated[
        float,
        ParamInfo('Pixel size of image [Angstroms] (known via rln_DetectorPixelSize*10e6 / rln_Magnification)')] = 0.0
    ms_estimated_resolution: Annotated[float, ParamInfo('Estimated resolution [Angstroms]')] = 0.0
    particle_diameter: Annotated[float, ParamInfo('Diameter of macromolecule [Angstroms]')] = 0.0
    aperture_index: Annotated[int, ParamInfo('Aperture index {1,2,3...}; increases tessellated bin size')] = 1

    # tessellation binning:
    prd_thres_low: Annotated[int,
                             ParamInfo('Minimum required snapshots in a tessellation for it be admitted', True,
                                       [ProjectLevel.BINNING])] = 100
    prd_thres_high: Annotated[int,
                              ParamInfo('Maximum number of snapshots that will be considered within each tessellation',
                                        True, [ProjectLevel.BINNING])] = 2000
    prd_n_active: Annotated[int, ParamInfo('Total number of bins to consider for manifold embedding')] = 0
    tess_hemisphere_vec: Annotated[
        ...
        ParamInfo(
            ...
            'PrDs opposite this plane will be mirrored', True, [ProjectLevel.BINNING])] = [1.0, 0.0, 0.0]
    tess_hemisphere_type: Annotated[
        str,
        ParamInfo('Technique to tesselate sphere. Valid options: ["lovisolo_silva", "fibonacci"]', True,
                  [ProjectLevel.BINNING])] = "lovisolo_silva"

    # Distance calculation parameters
    distance_filter_type: Annotated[str,
                                    ParamInfo('Filter type for image preprocessing. Valid: {"Butter", "Gauss"}', True,
                                              [ProjectLevel.CALC_DISTANCE])] = 'Butter'
    distance_filter_cutoff_freq: Annotated[
        float, ParamInfo('Nyquist cutoff frequency for filter', True, [ProjectLevel.CALC_DISTANCE])] = 0.5
    distance_filter_order: Annotated[
        int, ParamInfo('Order of Filter ("Butter" only)', True, [ProjectLevel.CALC_DISTANCE])] = 8

    # eigenfunction parameters:
    num_eigs: Annotated[
        int,
        ParamInfo('Number of highest-eigenvalue eigenfunctions to consider in total (max entry of eigenvalue spectrum)'
                  )] = 15
    num_psi_truncated: Annotated[int, ParamInfo('Number of eigenfunctions for truncated views')] = 8
    num_psi: Annotated[int, ParamInfo('Number of eigenfunctions for analysis', True, [ProjectLevel.CALC_DISTANCE])] = 8
    rad: Annotated[int, ParamInfo('Manifold pruning'), True, [ProjectLevel.MANIFOLD_ANALYSIS]] = 5

    # NLSA parameters:
    nlsa_fps: Annotated[float,
                        ParamInfo('Frames per second for generated movies', True, [ProjectLevel.NLSA_MOVIE])] = 5.0
    con_order_range: Annotated[
        int,
        ParamInfo('Coarse-graining factor of energy landscape', True,
                  [ProjectLevel.PSI_ANALYSIS, ProjectLevel.ENERGY_LANDSCAPE, ProjectLevel.TRAJECTORY])] = 50
    ...
    nlsa_tune: Annotated[int,
                         ParamInfo('Diffusion map tuning parameter', True,
    ...
    temperature: Annotated[int,
                           ParamInfo('User-defined pre-quenching temperature of experiments (Celcius)'), True,
                           [ProjectLevel.ENERGY_LANDSCAPE]] = 25
    traj_name: Annotated[str, ParamInfo('Filename modifier for exported (2D) trajectories')] = '1'
    states_per_coord: Annotated[
        int,
        ParamInfo('Number of states partitioned within each 1D reaction coordinate; results in a nClassx1 1D ELS'
                  )] = 50
    ...
    opt_movie: dict = {'printFig': 0, 'OFvisual': 0, 'visual_CC': 0, 'flowVecPctThresh': 95}
    ...
    def ang_width(self) -> float:
        if not self.particle_diameter:
            return 0.0
        return np.min(
            ((self.aperture_index * self.ms_estimated_resolution) / self.particle_diameter, np.sqrt(4 * np.pi)))
    ...
    def get_params_for_level(self,
                             level: ProjectLevel,
                             first_appearance=True,
                             user_only=True) -> dict[str, tuple[type, ParamInfo]]:
