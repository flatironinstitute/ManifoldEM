...
def psi_analysis_single(input_data, con_order_range, traj_name, is_full, psi_trunc):
    psi2_file = input_data[0]  # output to be generated by Psi Analysis
    EL_file = input_data[1]
    psinums = input_data[2]
    senses = input_data[3]
    prd = input_data[4]
    if len(input_data) == 6:
        psi_list = input_data[5]
    else:
        psi_list = psinums
    ...
    data_psi = data_store.get_diff_maps()

    psi = data_psi.psi(prd)  # coordinates of all images in 15-dim space from diffusion map: e.g., shape=(numPDs,15)
    pos_path = data_psi.pos_path(prd)  # indices of every image in PD: e.g., shape=(numPDs,); [0,1,2,...(numPDs-1)]
...
def _construct_input_data(N):
    ...
    for prD in range(N):
        ...
        ll.append([psi2_file, EL_file, psinums, senses, prD, psi_list])
    return ll
