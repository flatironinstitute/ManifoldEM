...
class Erg1dMain(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.entry_width.setToolTip(
            "Change the range of neighboring states to average for final reconstruction."
        )
...
class Erg1dCanvas(FigureCanvas):
    ...
    def update_figure(self, plot_occupancies=False, CC_coord=1):
        key = "occupancy" if plot_occupancies else "energy"
        LS1d = np.load(params.energy_landscape_file)[key]
        ...
        if plot_occupancies:
            self.axes.plot(
                np.arange(1, len(LS1d) + 1), LS1d, linewidth=1, c="#d62728"
            )  # C2
            self.axes.set_title("1D Occupancy Map", fontsize=8)
            self.axes.set_ylabel("Occupancy", fontsize=6)
        else:
            self.axes.plot(
                np.arange(1, len(LS1d) + 1), LS1d, linewidth=1, c="#1f77b4"
            )  # C0
