...
class _EigenSpectrumCanvas(FigureCanvas):
    ...
    def load_eigvals(self, index: int):
        ...
        lamb = np.array(data_store.get_embedding_data(index)["lamb"])[1:]
        col1 = np.arange(1, len(lamb) + 1)
        col2 = lamb
    ...
    def plot(self, index: int):
        ...
        self.axes.clear()
        self.axes.bar(
            self.eig_n1, self.eig_v1, edgecolor="none", color="#1f77b4", align="center"
        )  # C0: blue
        self.axes.bar(
            self.eig_n2, self.eig_v2, edgecolor="none", color="#7f7f7f", align="center"
        )  # C7: gray
        ...
        self.axes.axhline(0, color="k", linestyle="-", linewidth=0.25)
        self.axes.get_xaxis().set_tick_params(direction="out", width=0.25, length=2)
        self.axes.get_yaxis().set_tick_params(direction="out", width=0.25, length=2)
