...
class _BandwidthCanvas(FigureCanvas):
    ...
    def plot(self, index: int):
        data = data_store.get_embedding_data(index)

        logEps = np.array(data["logEps"])
        logSumWij = np.array(data["logSumWij"])
        popt = np.array(data["popt"])
        R_squared = np.array(data["R_squared"])
        ...
        self.axes.clear()
        self.axes.scatter(
            logEps, logSumWij, s=1, c="C0", edgecolor="C0", zorder=0.1, label="data"
        )
        self.axes.plot(
            logEps,
        ...
        self.axes.set_ylabel(
            r"$\mathrm{ln \ \sum_{i,j} \ A_{i,j}}$", fontsize=8, rotation=90
        )
