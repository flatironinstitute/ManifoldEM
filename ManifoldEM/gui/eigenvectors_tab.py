...
def get_blank_pixmap(path: str):
    ...
    blank.fill(0)
    blank = QImage(
        blank, blank.shape[1], blank.shape[0], blank.shape[1] * 3, QImage.Format_RGB888
    )
...
class EigenvectorsTab(QWidget):
    def __init__(self, parent):
        ...
        self.entry_prd.setToolTip(
            "Change the projection direction of the current view above."
        )
        ...
        self.trash_selector.setToolTip(
            "Check to remove the current PD from the final reconstruction."
        )
        ...
        self.label_reactCoord.setAlignment(
    ...
    def view_bandwidth(self):
        ...
        self.bandwidth_window.setWindowTitle(
            f"Projection Direction {self.user_prd_index}"
        )
    ...
    def view_eigspec(self):
        ...
        self.eigspec_window.setWindowTitle(
            f"Projection Direction {self.user_prd_index}"
        )
    ...
    def view_cc_details(self, psi_index):
        ...
        self.cc_details_window.setWindowTitle(
            "PD %s: Psi %s" % (self.user_prd_index, psi_index)
        )
    ...
    def update_pd_view(self):
        ...
        self.viz2.update_view(
            azimuth=phi, elevation=theta, distance=self.viz2.view_angles()
        )
    ...
    def update_psi_view(self):
        ...
        for i, (label, button) in enumerate(zip(self.label_pic, self.button_pic)):
            ...
            topo = (255.0 * (topo - topo.min()) / (topo.max() - topo.min())).astype(
                np.uint8
            )
            topo = QImage(topo, topo.shape[1], topo.shape[0], QImage.Format_Grayscale8)
            label.setPixmap(QPixmap(topo))
    ...
    def finalize(self):
        ...
        if len(prds.anchors) < min_allowed_anchors:
            ...
            box.setInformativeText(
                f"A minimum of {min_allowed_anchors} PD anchors must be selected."
            )
        ...
        if anchor_colors == all_colors:
            ...
            msg = (
                "Performing this action will initiate Belief Propagation for the current"
            ...
            msg = (
                "It is highly recommended that at least one anchor node is selected for each connected "
