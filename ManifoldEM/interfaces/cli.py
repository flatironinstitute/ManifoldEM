...
def get_parser():
        ...
        for param, (paramtype, paraminfo) in annotated_params.items():
            if paraminfo.user_param:
                ...
                subparser.add_argument(
                    f"--{param}",
                    metavar=paramtype.__name__.upper(),
                    type=paramtype,
                    default=default,
    ...
    init_parser.add_argument(
        "-v", "--avg-volume", type=str, metavar="FILEPATH", default=""
    )
    init_parser.add_argument(
        "-a", "--alignment", type=str, metavar="FILEPATH", default=""
    )
    init_parser.add_argument(
        "-i", "--image-stack", type=str, metavar="FILEPATH", default=""
    )
    init_parser.add_argument(
        "-m", "--mask-volume", type=str, metavar="FILEPATH", default=""
    )
    init_parser.add_argument(
        "-s", "--pixel-size", type=float, metavar="FLOAT", required=True
    )
    init_parser.add_argument(
        "-d", "--diameter", type=float, metavar="FLOAT", required=True
    )
    init_parser.add_argument(
        "-r", "--resolution", type=float, metavar="FLOAT", required=True
    )
    init_parser.add_argument(
        "-x", "--aperture-index", type=int, metavar="INT", default=1
    )
    init_parser.add_argument(
        "-o",
        "--overwrite",
    ...
    threshold_parser = subparsers.add_parser(
        "threshold",
        help="1: Set upper/lower thresholds for principal direction detection",
        formatter_class=ArgumentDefaultsHelpFormatter,
    )
    ...
    decompose_parser = subparsers.add_parser(
        "decompose",
        help="2: Embed images onto manifold and decompose",
        formatter_class=ArgumentDefaultsHelpFormatter,
    )
    decompose_parser.add_argument("input_file", type=str)
    decompose_parser.add_argument(
        "--prds",
        type=str,
        metavar="INT,INT,...",
        help="Comma delineated list of prds you wish to calculate -- useful for debugging",
    )
    add_relevant_params(decompose_parser, ProjectLevel.DECOMPOSITION)

    cc_parser = subparsers.add_parser(
        "find-ccs",
        help="3: Find conformational coordinates",
        formatter_class=ArgumentDefaultsHelpFormatter,
    )
    ...
    el_parser = subparsers.add_parser(
        "energy-landscape",
        help="4: Calculate energy landscape",
        formatter_class=ArgumentDefaultsHelpFormatter,
    )
    ...
    mrcs2mrc_parser = utility_subparsers.add_parser(
        "mrcs2mrc",
    ...
    denoise_parser = utility_subparsers.add_parser(
        "denoise",
        help="Denoise output of mrcs2mrc postprocessing step",
        formatter_class=ArgumentDefaultsHelpFormatter,
    )
    ...
    denoise_parser.add_argument(
        ...
        metavar="INT",
        default=5,
        help="Beginning and ending frames affected",
    )
    denoise_parser.add_argument(
        "--filter",
        type=str,
        metavar="STR",
        default="Gaussian",
        help="Filter type: {Gaussian, Median}",
    )
...
def decompose(args: Namespace):
    mem.decompose(**args_to_dict(args))
...
def relion_reconstruct(star_file: str, relion_command: str, output_path: str):
    ...
    subprocess.run(
        [relion_command, "--i", star_file, "--o", mrc_file], capture_output=True
    )
...
def mrcs2mrc(_):
    ...
    if not relion_command:
        print(
            "Can't find relion command 'relion_reconstruct'. Please verify that it is in your path"
        )
        return
    ...
    reconstruct_local = partial(
        relion_reconstruct, relion_command=relion_command, output_path=output_path
    )
    ...
    with multiprocessing.Pool(processes=params.ncpu) as pool:
        for _ in tqdm.tqdm(
            enumerate(pool.imap_unordered(reconstruct_local, star_files)),
...
def denoise_helper(i_bin: int, k: int, filter_type: str):
    ...
    rec_file = os.path.join(
        params.postproc_mrcs2mrc_dir,
        "EulerAngles_{}_{}_of_{}.mrc".format(
            params.traj_name, i_bin + 1, params.states_per_coord
        ),
    )
    ...
    rec1_file = os.path.join(
        params.postproc_denoise_dir,
        "DenoiseimgsRELION_{}_{}_of_{}.mrc".format(
            params.traj_name, i_bin + 1, params.states_per_coord
        ),
    )
...
def denoise(args):
    ...
    bins = list(range(f)) + list(
        range(params.states_per_coord - f, params.states_per_coord)
    )
    ...
    with multiprocessing.Pool(processes=params.ncpu) as pool:
        for _ in tqdm.tqdm(
            enumerate(pool.imap_unordered(denoise_local, bins)), total=len(bins)
        ):
            pass
