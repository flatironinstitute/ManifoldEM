...
from ManifoldEM import q2Spider
...
def eulerRotMatrix3DSpider(Phi, Theta, Psi, deg):
    ...
    R = np.array([[
        cos(Phi) * cos(Psi) * cos(Theta) + (-1) * sin(Phi) * sin(Psi),
        cos(Psi) * cos(Theta) * sin(Phi) + cos(Phi) * sin(Psi), (-1) * cos(Psi) * sin(Theta)
    ],
                  [(-1) * cos(Psi) * sin(Phi) + (-1) * cos(Phi) * cos(Theta) * sin(Psi),
                   cos(Phi) * cos(Psi) + (-1) * cos(Theta) * sin(Phi) * sin(Psi),
                   sin(Psi) * sin(Theta)], [cos(Phi) * sin(Theta),
                                            sin(Phi) * sin(Theta),
                                            cos(Theta)]])
